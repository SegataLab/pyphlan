...
class PpaTree:
    ...
    def _find_core( self, terminals, er = 0.95, root_name = None, skype_qm = True ):
            ...
            if clade.is_terminal():
                if clade.imgid in terminals:
                    ...
                    return [(clname,1,1,
                                ...
                                1.0)]
                return []
            if skype_qm and clade.name and "?" in clade.name:
                return [] 
    ...
    def find_cores( self, cl_taxa_file, min_core_size = 1, error_rate = 0.95, subtree = None, skype_qm = True ):
        if subtree:
        ...
        for vec in (l.strip().split('\t') for l in open(cl_taxa_file)):
            ...
            if len(tgts) >= min_core_size:
                ...
                ret[sid] = self._find_core( tgts, er = error_rate, root_name = subtree, skype_qm = skype_qm )
                #print sid #, ret[sid]
        return ret
